...
import { UserPlus, Bike, UserCog } from "lucide-react";

export default function AdminStaff() {
  const [form, setForm] = useState({ name: "", phone: "", email: "", password: "", role: "LIVREUR" });
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setForm({ name: "", phone: "", email: "", password: "", role: "LIVREUR" });
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                <button type="button" onClick={() => setForm({ ...form, role: "LIVREUR" })} className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium ${form.role === "LIVREUR" ? "border-[#e67e22] bg-orange-50 text-[#e67e22]" : "border-gray-200"}`}>
                  <Bike className="w-4 h-4" /> Livreur
                </button>
                <button type="button" onClick={() => setForm({ ...form, role: "GERANT" })} className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium ${form.role === "GERANT" ? "border-[#e67e22] bg-orange-50 text-[#e67e22]" : "border-gray-200"}`}>
