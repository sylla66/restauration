...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    dashboard.sales({ period: "day" }).then(setSales).catch(() => {});
    dashboard.topItems({ period: "day" }).then((r) => setTopItems(r.topItems || [])).catch(() => {});
    dashboard.cancellations({ period: "day" }).then((r) => setCancellations(r)).catch(() => {});
    dashboard.deliveryTimes({ period: "day" }).then((r) => setDeliveryTimes(r)).catch(() => {});
      ...
      <Card>
        ...
        <CardContent>
          {recentOrders.length === 0 ? (
            <p className="text-[var(--muted-foreground)] text-center py-4">Aucune commande récente</p>
          ) : (
            <div className="space-y-2">
              {recentOrders.map((o) => (
                <div key={o.id} className="flex justify-between items-center text-sm p-2 hover:bg-[var(--muted)] rounded-lg cursor-pointer" onClick={() => navigate(`/orders/${o.id}`)}>
                  <span className="font-medium">#{o.orderNumber || o.id.slice(0, 8)}</span>
                  <span className="text-[var(--muted-foreground)]">{new Date(o.createdAt).toLocaleTimeString("fr-FR")}</span>
